...
- **Twelve faces**, full-screen layouts with many values and their own
  animation: cockpit (liquid, CPU/GPU temperatures, RAM, VRAM, load per
  core, three charts, clock), helm (the same values around the rim of
  the round display, one bar per core), orbit, duo (CPU against GPU),
  history (liquid over 24 h with minimum, mean and maximum), fire and
  caustics with secondary values, video, clock, music (an example track for now),
  alarm (only while a temperature is over its limit) and night (only
  during its hours). Words in English or German (`render.language`).
...
- **Backgrounds:** drop GIFs into the assets directory (`liquid.gif`,
  ...
  automatically — the cache tracks file identity. A tile keeps at most
  `max_frames` frames of its GIF and drops the rest, so a longer GIF
  loses the end of its loop. The shipped backgrounds (pool caustics, a
  ring of fire, a memory die with data pulses, a spinning wireframe
  globe, a plasma globe, an evening lake) are
  generated by `scripts/generate_backgrounds.py` (needs numpy, one module
  per scene in `scripts/backgrounds/`); tweak their palettes and rebuild
  your own.
...
```bash
python3 -m pytest      # 260 tests, no hardware required
...
- [docs/ROADMAP.md](docs/ROADMAP.md) — the way to 2.0: free layouts,
  history, rules, media and an app, with hardware gates for every new
  display path.
